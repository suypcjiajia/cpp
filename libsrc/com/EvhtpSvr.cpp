...
#include "/home/cpp/shareserver/phonesms/src/dal/MyDAL.h"
...
{
    //配置文件
    const string sConfig = "/tiyoume/"+sServiceName+"/appid.cnf";
    //读配置文件
    TFile tFile;
...
//启动服务
void EvhtpSvr::StartSvr(ProcessCallBack pProcessCallBack,const int piPort,const string& psIsSSL,const string& psThreads,const string& psIsAes,const string& psIsCheckSafe){
    msIsCheckSafe = psIsCheckSafe;
    msIsAes = psIsAes;
    mProcessCallBack = pProcessCallBack;
    try{
...
void EvhtpSvr::Process2(evhtp_request_t * request, void * pUserParma)
{
    ...
    try{
        ...
        RetJsonMsg::SetRetMsg2(-1,jsonRespose);
        ...
        if( !sToken.empty() ){
            string sAppKey = "tym1234567890123tym1234567890123";
            MyAES aes;
            aes.SetKey((unsigned char*)sAppKey.c_str(),sAppKey.length());
            sDeToken = aes.Decrypt(sToken,0);
            mapHeadIn["detoken"] = sDeToken;
        }
        ...
        if(htp_method_POST ==  eMethod ){// process Post
            ...
            appendlog(TTDLogger::LOG_INFO,"Url=%s,PostData=%s" ,request->uri->path->full,sPostData.c_str() );
            ...
            if(  0 == iRet){
                ...
                if (mProcessCallBack2 != NULL) {
                    ...
                    //mProcessCallBack2(sUrlPath,sDeToken,jsonValue,jsonRespose);
                    mProcessCallBack2(this,jsonValue,jsonRespose);
                }
            }else{
                RetJsonMsg::SetRetMsg2(-20,jsonRespose);
            }
            ...
            RetJsonMsg::SetRetMsg2(-13,jsonRespose);
        }
...
void EvhtpSvr::Process(evhtp_request_t * request, void * pUserParma){
    ...
    try{
        ...
        string sMode = mapQueryParam["sMode"];
        bool bAes = true;
        if( sMode == "1"){
            bAes = false;
        }
        ...
        if(htp_method_POST ==  eMethod ){// process Post
            ...
            //安全检验
            if(SafeCheck(sAppid,sTimeStamp,sSign,sUrlDecodePost, bAes) == -1){
                //RetJsonMsg::SetRetMsg(-14,jsonRespose);
                goto RESPONSE;
            }else{
                string  sPostData;

                if( bAes && (msIsAes == "true")){
                ...
                if(  0 == iRet){
                    ...
                    if (mProcessCallBack != NULL) {
                        mProcessCallBack(this,jsonValue,jsonRespose);
                    }
                }else{
                    jsonRespose.clear();
...
//安全检验
int EvhtpSvr::SafeCheck(const string & psAppid,const string & psTimeStamp,const string & psSign,const string & psPostData,bool bAes){
